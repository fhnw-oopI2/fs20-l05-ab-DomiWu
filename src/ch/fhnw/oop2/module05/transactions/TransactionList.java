...
public final class TransactionList {
	...
	public List<Transaction> transactionsInYear(int year) {
		return allTransactions.stream().filter(transaction -> transaction.getYear() == year)
	...
	public List<String> cities() {
		return allTransactions.stream().map(transaction -> transaction.getTrader().getCity()).distinct()
	...
	public List<Trader> traders(String city) {
		return allTransactions.stream().filter(transaction -> transaction.getTrader().getCity().equals(city))
				.map(t -> t.getTrader()).distinct().sorted(Comparator.comparing(t -> t.getName()))
				.collect(Collectors.toList());
	}
	...
	public boolean traderInCity(String city) {
		return 0 < allTransactions.stream().filter(transaction -> transaction.getTrader().getCity().equals(city))
				.count();
	}
	...
	public void relocateTraders(String from, String to) {
		allTransactions.forEach(transaction -> {
			if (transaction.getTrader().getCity().equals(from)) {
				transaction.getTrader().setCity(to);
			}
		});
	}
	...
	public int highestValue() {
		return allTransactions.stream().collect(Collectors.summarizingInt(t -> t.getValue())).getMax();
	}
}